...
contract sFyde is IsMGMT, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgMGMT public gMGMT; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked MGMT", "sMGMT", 9) ERC20Permit("Staked MGMT") {
    ...
    function setgMGMT(address _gMGMT) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gMGMT) == address(0), "gMGMT:  gMGMT already set");
        require(_gMGMT != address(0), "gMGMT:  gMGMT is not a valid contract");
        gMGMT = IgMGMT(_gMGMT);
    }
    ...
    /**
        @notice increases rMGMT supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sMGMT of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sMGMT
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sMGMT: insufficient balance");
    }
    ...
    // toG converts an sMGMT balance to gMGMT terms. gMGMT is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gMGMT.balanceTo(amount);
    }

    // fromG converts a gMGMT balance to sMGMT terms. sMGMT is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gMGMT.balanceFrom(amount);
    }

    // Staking contract holds excess sMGMT
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gMGMT.balanceFrom(IERC20(address(gMGMT)).totalSupply())).add(
