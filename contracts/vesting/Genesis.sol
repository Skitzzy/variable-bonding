...
/**
 *  This contract allows Fyde genesis contributors to claim MGMT. It has been
...
contract GenesisClaim is Ownable {
    ...
    function _claim(uint256 _amount) internal returns (uint256 toSend_) {
        ...
        if (useStatic) {
            terms[msg.sender].gClaimed = info.gClaimed.add(gMGMT.balanceTo(toSend_.mul(9).div(10)));
            terms[msg.sender].claimed = info.claimed.add(toSend_.div(10));
        } else terms[msg.sender].gClaimed = info.gClaimed.add(gMGMT.balanceTo(toSend_));
    }
    ...
    function claimed(address _address) public view returns (uint256) {
        return gMGMT.balanceFrom(terms[_address].gClaimed).add(terms[_address].claimed);
    }
