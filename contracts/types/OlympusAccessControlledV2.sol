...
abstract contract FydeAccessControlledV2 {
    /* ========== EVENTS ========== */

    event AuthorityUpdated(IFydeAuthority authority);
    ...
    function initializeAuthority(IFydeAuthority _newAuthority) internal {
        if (authority != IFydeAuthority(address(0))) revert AUTHORITY_INITIALIZED();
        authority = _newAuthority;
        emit AuthorityUpdated(_newAuthority);
    }

    function setAuthority(IFydeAuthority _newAuthority) external {
        _onlyGovernor();
        authority = _newAuthority;
        emit AuthorityUpdated(_newAuthority);
    }
