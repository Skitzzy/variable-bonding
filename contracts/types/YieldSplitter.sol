...
import {FydeAccessControlledV2, IFydeAuthority} from "../types/FydeAccessControlledV2.sol";

/**
    @title IMGMTIndexWrapper
    @notice This interface is used to wrap cross-chain oracles to feed an index without needing IsMGMT, 
    while also being able to use sMGMT on mainnet.
 */
interface IMGMTIndexWrapper {
...
/**
    @title YieldSplitter
    @notice Abstract contract that allows users to create deposits for their gMGMT and have
            ...
            emergency controls, sending and recieving gMGMT is up to the implementation of
            this abstract contract to handle.
 */
abstract contract YieldSplitter is FydeAccessControlledV2, IYieldSplitter {
    using SafeERC20 for IERC20;

    IMGMTIndexWrapper public immutable indexWrapper;

    struct DepositInfo {
        uint256 id;
        address depositor;
        uint256 principalAmount; // Total amount of sMGMT deposited as principal, 9 decimals.
        uint256 agnosticAmount; // Total amount deposited priced in gMGMT. 18 decimals.
    }
    ...
    /**
        @notice Constructor
        @param indexWrapper_ Address of contract that will return the sMGMT to gMGMT index. 
                             On mainnet this will be sMGMT but on other chains can be an oracle wrapper.
    */
    constructor(address indexWrapper_, address authority_) FydeAccessControlledV2(IFydeAuthority(authority_)) {
        indexWrapper = IMGMTIndexWrapper(indexWrapper_);
    }
    ...
    */
    ...
    /**
        @notice Convert flat sMGMT value to agnostic gMGMT value at current index
        @dev Agnostic value earns rebases. Agnostic value is amount / rebase_index.
             1e18 is because sMGMT has 9 decimals, gMGMT has 18 and index has 9.
     */
    ...
    /**
        @notice Convert agnostic gMGMT value at current index to flat sMGMT value
        @dev Agnostic value earns rebases. sMGMT amount is gMGMTamount * rebase_index.
             1e18 is because sMGMT has 9 decimals, gMGMT has 18 and index has 9.
     */
