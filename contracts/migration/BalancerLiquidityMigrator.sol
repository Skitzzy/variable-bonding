...
contract BalancerLiquidityMigrator is FydeAccessControlled {
    ...
    // Balancer 50MGMT-25DAI-25WETH poolID
    ...
    address internal immutable MGMTDAISLP = 0x055475920a8c93CfFb64d039A8205F7AcC7722d3;
    ...
    /**
     * @notice Removes liquidity from MGMT/ETH SLP and MGMT/DAI SLP, then adds liquidty to
     * 50MGMT-25DAI-25WETH Balancer pool.
     */
    function moveLiquidity(
        uint256 _amountMGMTETH,
        uint256 _amountMGMTDAI,
        uint256[2] memory _minMGMTETH,
        uint256[2] memory _minMGMTDAI,
        uint256 _deadline,
        bytes memory _userData
    ) external onlyGuardian {
        // Manage LPs from treasury
        treasury.manage(MGMTETHSLP, _amountMGMTETH);
        treasury.manage(MGMTDAISLP, _amountMGMTDAI);

        // Approve LPs to be spent by the Sushiswap router
        IERC20(MGMTETHSLP).approve(address(router), _amountMGMTETH);
        IERC20(MGMTDAISLP).approve(address(router), _amountMGMTDAI);

        // Remove specified liquidity from MGMT/ETH SLP
        (uint256 amountETH, uint256 amountMGMT1) = router.removeLiquidity(
            WETH,
            MGMT,
            _amountMGMTETH,
            _minMGMTETH[0],
        ...
        // Remove specified liquidity from MGMT/DAI SLP
        (uint256 amountDAI, uint256 amountMGMT2) = router.removeLiquidity(
            DAI,
            MGMT,
            _amountMGMTDAI,
            _minMGMTDAI[0],
        ...
        // Amount of MGMT removed from liquidity
        uint256 amountMGMT = amountMGMT1 + amountMGMT2;

        // Approve Balancer vault to spend tokens
        IERC20(MGMT).approve(address(balancerVault), amountMGMT);
        ...
        // Send any leftover MGMT back to guardian and WETH and DAI to treasury
