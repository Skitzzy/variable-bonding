...
contract LUSDSwapContract is FydeAccessControlled {
    ...
    /// @notice Remaining amount of DAI to have each MGMT V1 backed by 1 DAI;
    uint256 public MGMTV1BackingInDAIRemaining;
    ...
    constructor(IFydeAuthority _authority) FydeAccessControlled(_authority) {
        MGMTV1BackingInDAIRemaining = MGMTV1.totalSupply() * 1e9;
    }
    ...
    function swapLUSDForDAI(
        ...
        if (_amountDAIToV1Treasury > 0) {
            if (MGMTV1BackingInDAIRemaining < _amountDAIToV1Treasury) revert OverMGMTV1Backing();
            IERC20(DAI).safeTransfer(address(treasuryV1), _amountDAIToV1Treasury);
            MGMTV1BackingInDAIRemaining -= _amountDAIToV1Treasury;
