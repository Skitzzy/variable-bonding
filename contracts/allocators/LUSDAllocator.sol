...
contract LUSDAllocator is FydeAccessControlled {
    /* ======== DEPENDENCIES ======== */
    ...
    ITreasury public treasury; // Fyde Treasury
    ...
    function withdraw(address token, uint256 amount) external onlyGuardian {
        ...
        if (token == lusdTokenAddress) {
            ...
            treasury.deposit(balance, token, value); // deposit using value as profit so no MGMT is minted
        } else {
    ...
    /**
    Helper method copying FydeTreasury::_tokenValue(), whose name was 'valueOf()' in v1 
